...
import { _SERVICE } from './dfx_generated/tapswap/tapswap.did';
...
export function getTests(helloWorldCanister: ActorSubclass<_SERVICE>): Test {
    return () => {
        it('gets original message', async () => {
            ...
            expect(result).toBe('tapswap!');
        });

        it('sets a new message', async () => {
            const result =
                await helloWorldCanister.setMessage('Goodbye tapswap!');
        ...
        it('gets persisted new message', async () => {
            ...
            expect(result).toBe('Goodbye tapswap!');
        });
    };
}
